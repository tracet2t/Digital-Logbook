...
describe("US41 - MentorFeedbackRepository.getMenteeFeedbackHistory", () => {
  ...
  test("returns mapped feedback history including status and feedback notes", async () => {
  ...
  test("returns an empty list when mentee has no feedback history", async () => {
    (prisma.mentorFeedback.findMany as jest.Mock).mockResolvedValue([]);
