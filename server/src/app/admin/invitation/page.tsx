...
import InvitationsTable, { InvitationRow } from '@/components/Invitations/InvitationsTable'
...
export default function InvitationsView() {
  ...
  const [formData, setFormData] = useState({ role: "", firstName: "", lastName: "", email: "", project: "" });
  ...
  const handleSubmit = () => {
    ...
    sendInvitation({
      ...
      onSuccess: () => {
        setFormData({ role: "", firstName: "", lastName: "", email: "", project: "" });
        setCreateOpen(false);
      }
    });
  };
  ...
  return (
    <>
      <div className="flex min-h-screen bg-[#f5f5f7]">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="bg-white border-b border-slate-200 px-8 py-6 flex items-center justify-between">
            ...
            <button onClick={() => setCreateOpen(true)} className="inline-flex items-center gap-2 h-9 px-4 bg-[#18181B] hover:bg-[#27272A] text-white text-sm font-medium rounded-lg transition-colors shadow-sm shrink-0">
          ...
          <div className="flex-1 px-8 py-6 space-y-6">
            {/* Stats */}
            <InvitationsStats loading={isInvLoading} total={totalInvitations} pending={pendingCount} accepted={acceptedCount} expired={expiredCount} />
            ...
            <InvitationsTable
              ...
              onView={setViewInv}
              onChangeStatus={(inv) => { setChangeStatusInv(inv); setSelectedStatus(inv.status); }}
      ...
      />
      <ViewInvitationDialog invitation={viewInv} onClose={() => setViewInv(null)} />
      <ChangeStatusDialog
        invitation={changeStatusInv}
        selectedStatus={selectedStatus}
        onChangeStatus={setSelectedStatus}
        onSave={() => changeStatusInv && changeStatus(
      ...
      <DeleteInvitationDialog
        ...
        onDelete={() => deleteId && deleteInvitation({ id: deleteId }, { onSettled: () => setDeleteId(null) })}
