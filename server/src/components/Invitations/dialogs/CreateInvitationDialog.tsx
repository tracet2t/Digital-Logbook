...
export default function CreateInvitationDialog({
  open, onOpenChange, formData, errors, projects, isPending, onChange, onSubmit
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl p-0 gap-0 overflow-hidden">
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                {errors.project ? <p className="text-xs text-red-500 font-medium">{errors.project}</p> : <p className="text-xs text-slate-400">Assign to an existing project</p>}
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-1.5 mt-4">
              ...
              />
              {errors.email ? <p className="text-xs text-red-500 font-medium">{errors.email}</p> : <p className="text-xs text-slate-400">The invitation link will be sent to this address</p>}
            </div>
          </div>
        </div>
        ...
        <div className="px-6 py-4 border-t border-slate-200 bg-slate-50 flex items-center justify-between gap-3">
          <DialogClose asChild>
            <Button variant="outline" disabled={isPending} className="h-9 px-4 text-sm">Cancel</Button>
          </DialogClose>
          <button
            onClick={onSubmit}
            disabled={isPending}
