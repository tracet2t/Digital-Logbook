...
export default function ChangeStatusDialog({ invitation, selectedStatus, onChangeStatus, onSave, isPending, onClose }: ChangeStatusDialogProps) {
    ...
    <Dialog open={!!invitation} onOpenChange={() => onClose()}>
      <DialogContent className="max-w-sm p-0 gap-0 overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-200 bg-slate-50">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs text-slate-500 mt-0.5">
              Update the status for <span className="font-medium text-slate-700">{invitation.email}</span>
            </DialogDescription>
          </DialogHeader>
        </div>
        ...
        <div className="px-5 py-3 border-t border-slate-200 bg-slate-50 flex items-center justify-end gap-2">
          <DialogClose asChild><Button variant="outline" disabled={isPending} className="h-8 px-3 text-sm">Cancel</Button></DialogClose>
          <button
