...
export default function InvitationsTable({ data, loading, error, onView, onChangeStatus, onDelete }: InvitationsTableProps) {
  return (
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-100 text-sm">
            {loading && <tr><td colSpan={5} className="px-5 py-8 text-center text-slate-400">Loading...</td></tr>}
            {error && <tr><td colSpan={5} className="px-5 py-8 text-center text-red-500">Failed to load invitations</td></tr>}
            ...
            {!loading && data.map((inv) => (
              <tr key={inv.id} className="hover:bg-slate-50/50 transition-colors">
                ...
                <td className="px-5 py-4">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide ${inv.role === 'mentor' || inv.role === 'superAdmin' ? 'bg-[#18181B] text-white' : 'bg-slate-100 text-slate-600'}`}>
                    {inv.role}
                  </span>
                </td>
                ...
                <td className="px-5 py-4 text-right">
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-44">
                      <DropdownMenuItem onSelect={() => onView(inv)}>View Details</DropdownMenuItem>
                      <DropdownMenuItem onSelect={() => onChangeStatus(inv)}>Change Status</DropdownMenuItem>
                      <DropdownMenuSeparator />
                      <DropdownMenuItem className="text-red-600 focus:text-red-600 focus:bg-red-50" onSelect={() => onDelete(inv.id)}>Delete</DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                </td>
              </tr>
